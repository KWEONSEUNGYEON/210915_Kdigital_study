...
namespace OracleDB_ParkingCar
{
    public class DataManager
    {
        ...
        static void ConnectDB() //참고로 앞에 아무것도 안붙이면 private이 붙은 거랑 똑같다.
        {
            try
            {
                OraConn.Open();
        ...
        public static void selectQuery()
        {
            ...
            oda.SelectCommand.CommandText = sql; //select 문을 날린거
            ...
            foreach (DataRow item in ds.Tables[0].Rows)
            {
                ...
                //ParkingTime 컬럼에 아무것도 없다면 ParkingTime 속성에 new DataTime() //(현재시간) 삽입
        ...
        //특정한 공간에 이미 주차되어 있거나 출차되어 있는 걸 체크하기 위해서 한 군데의 상태를 조회하는 함수
        //위의 selectQuery랑 이름은 똑같으낭 매개변수가 다르므로 다른 함수로 취급
        public static ParkingCar selectQuery(int spot)
        {
            ConnectDB();

            string sql;
            //select * from parkingcar where parkingspot=1 order by to_number(parkingspot)
        ...
        //executeQuery("update",1,"30규1234","권승연","010-7708-6396")
        //삽입, 삭제, 수정 역할
